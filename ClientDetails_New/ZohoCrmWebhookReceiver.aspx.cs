...
public partial class ZohoCrmWebhookReceiver : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else if (Request.HttpMethod == "GET")
        {
            try
            {
                ...
                string zcrmId = HttpUtility.UrlDecode(Decrypt(Request.QueryString["zcrmid"]));
                ...
                string module = HttpUtility.UrlDecode(Decrypt(Request.QueryString["module"]));
                ...
                if (!string.IsNullOrEmpty(token) && !string.IsNullOrEmpty(responder) && !string.IsNullOrEmpty(decision))
                {
                    ...
                    {
                        string CreatedBy = "", ModifiedBy = "", AccountName = "", AccountId = "", recordId = "", ZohoStatus = "", accOwnerEmail = "", JsonPayload = "";
                        ...
                        if (isFirstApproval)
                        {
                            using (SqlConnection conn = new SqlConnection(emailConnectionString))
                            {
                                ...
                                using (SqlDataReader reader = cmd.ExecuteReader())
                                {
                                    ...
                                    if (reader.Read())
                                    {
                                        ...
                                        JsonPayload = reader["JsonPayload"]?.ToString();
                                    }
                                }
                            }
                            ...
                            var WebhookData = JsonConvert.DeserializeObject<ZohoCrmWebhookData>(JsonPayload);
                            WebhookData.ModifiedBy = ModifiedBy;
                            ...
                            if (decision == "approve")
                            {
                                ...
                                UpdateZohoStatusById(emailConnectionString, recordId, "SUCCESS");
                                SendEmailApproval(JsonPayload, WebhookData, accOwnerEmail, isApprovalEmail, toRecipients, ccRecipients, bccRecipients, decision, "SUCCESS", recordId);
                            }
                            else
                            {
                                UpdateZohoStatusById(emailConnectionString, recordId, "REJECTED");
                                SendEmailApproval(JsonPayload, WebhookData, accOwnerEmail, isApprovalEmail, toRecipients, ccRecipients, bccRecipients, decision, "REJECTED", recordId);
                                Console.WriteLine("Rejected");
                            }

                            ZohoStatus = GetZohoStatusById(emailConnectionString, token, module);
                            ...
                            if (!string.IsNullOrEmpty(decision))
                            {
                                if (decision.Equals("Approve", StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        swalTitle = "❗Error";
                                        headerColor = "#d9534f"; // red
                                        headerText = "Error while finding the result";
                                    }
                                }
                                ...
                                <body>
                                    <div class='container' id='response-container'>
                                        ...
                                        <table>
                                            ...
                                            <tr>
                                                <td class='label'>Application Update Status</td>
    ...
    private static (string toMails, string ccMails, string bccMails) GetEmailApprovers(string connectionString, string Category, bool isApprovalEmail)
    {
    ...
    private void SendEmailApproval(string JsonPayload, ZohoCrmWebhookData data, string accOwnerEmail, bool isApprovalEmail, string toRecipients, string ccRecipients, string bccRecipients, string responderAction = null, string zohoResult = null, string recordId = null) // New Changes
    {
        string smtpHost = "";
        int smtpPort = 0;
        string smtpEmail = "";
        ...
        string action = "ZOHO CRM TO PED AND CMS";
    ...
    public static (string htmlBody, string subject, ZohoCrmWebhookData zohoData) EmailContentGenerator(ZohoCrmWebhookData data, string type, string module, string webhookLink, string token, string toEmail, string action, string formattedDate, bool isToRecipient, bool isApprovalEmail, string responderAction = null, string zohoResult = null)
    {
        string htmlBody = "";
        string subject = "";
        ...
        if ((data.Type == "EDIT" && (hasAnyContactChange || hasAnyAddressChange)) || data.Type == "ADD" || data.Type == "DELETE")
        {
                      ...
                      <table width=""800"" cellpadding=""0"" cellspacing=""0"" border=""0"" style=""width: 980px; border: solid 1px teal; border-radius: 5px; max-width: auto; margin: 0 auto;"">
                        ...
                        <tr>
                          <td align=""center"" style=""font-size: 18px; font-weight: bold; padding-bottom: 10px;"">
                            {(isApprovalEmail ? "APPROVAL REQUEST" : "CHANGES FROM")} ({action}) - ({type})
                          </td>
                        </tr>
                        <tr>
            ...
            {
                subject = $"Approval Needed ({action}) for Account: {data.AccountName} - {formattedDate}";
                htmlBody += $@"<!-- Buttons -->
                        <tr>
                          <td style=""padding: 10px;"">
                            ...
                            <table cellpadding=""0"" cellspacing=""0"" border=""0"">
                              <tr>
                                <td style=""padding-right: 10px;"">
                                  <a href=""{webhookLink}?token={HttpUtility.UrlEncode(Encrypt(token))}&zcrmid={HttpUtility.UrlEncode(Encrypt(data.ContactId))}&responder={HttpUtility.UrlEncode(Encrypt(toEmail))}&decision={HttpUtility.UrlEncode(Encrypt("approve"))}&module={HttpUtility.UrlEncode(Encrypt(data.Module))}""
                                     style=""display: inline-block; padding: 12px 20px; background-color: #28a745; color: #ffffff; text-decoration: none; border-radius: 4px;"">
                                    ✅ Approve
                                  </a>
                                </td>
                                <td>
                                  <a href=""{webhookLink}?token={HttpUtility.UrlEncode(Encrypt(token))}&zcrmid={HttpUtility.UrlEncode(Encrypt(data.ContactId))}&responder={HttpUtility.UrlEncode(Encrypt(toEmail))}&decision={HttpUtility.UrlEncode(Encrypt("reject"))}&module={HttpUtility.UrlEncode(Encrypt(data.Module))}""
            ...
            {
                subject = $"Changes from ({action}) for Account: {data.AccountName} - {formattedDate}";
            }
        ...
        return (htmlBody, subject, zohoData);

    }


    private void SaveToEmailApprovalTable(string JsonPayload, ZohoCrmWebhookData data, ZohoCrmWebhookData ZohoData, string accOwnerEmail, string token, string emailStatus, string errorMsg, string allRecipients, bool isApprovalEmail, string recordId = null)
    {
        if (isApprovalEmail)
        {
            using (SqlConnection connection = new SqlConnection(emailConnectionString))
            {
                connection.Open();
        ...
        else
        {
            if (!string.IsNullOrEmpty(recordId))
            {
                using (SqlConnection connection = new SqlConnection(emailConnectionString))
                {
                    ...
                    // Use stored procedure name instead of raw SQL
                    string storedProcedureName = "";
                    storedProcedureName = "UpdateConfirmEmailStatus";
            ...
            {
                Console.WriteLine($"Record id is empty.The value: {recordId}");
            }
    ...
    public static void UpdateZohoStatusById(string connectionString, string recordId, string status)
    {
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[UpdateZohoStatus]", conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@ZohoStatus", status);
    ...
    public static string GetZohoStatusById(string connectionString, string token, string module)
    {
        string result = "";
        try
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("[dbo].[GetZohoStatus]", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@Token", token);
                    cmd.Parameters.AddWithValue("@Module", module.ToUpper());
