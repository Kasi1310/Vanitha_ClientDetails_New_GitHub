...
{
    public partial class frmClientDigest : System.Web.UI.Page
    {
        clsClientDigest objclsClientDigest;
        ...
        {
            objclsClientDigest = new clsClientDigest();
            objclsClientDigest.Name = txtName.Text.Trim();
            objclsClientDigest.MailID = txtMailID.Text.Trim();
            objclsClientDigest.Comments = txtComments.Text.Trim();
            objclsClientDigest.InsertClientDigest();

            string To = ConfigurationManager.AppSettings["Digest.To.email"].ToString();
            string CC = ConfigurationManager.AppSettings["Digest.CC.email"].ToString();
            string BCC = ConfigurationManager.AppSettings["Digest.BCC.email"].ToString();

            clsSendMail objclsSendMail = new clsSendMail();
            objclsSendMail.SendMail(To, CC, BCC, "MEDICOUNT DIGEST", MailBody(txtName.Text.Trim(), txtMailID.Text.Trim(), txtComments.Text.Trim()));
        ...
        private string MailBody(string Name, string MailID, string Comments)
        {
            ...
            sb.AppendLine("<p align='center' style='margin: 0in; text - align:center; background:#009094'><b><span style='font-size:30.0pt;color:white'>MEDICOUNT DIGEST 2023</span></b><o:p></o:p></p>");
            ...
            sb.AppendLine("<tr>");
            sb.AppendLine("<td>");
            sb.AppendLine("Share your thoughts:");
            sb.AppendLine("</td>");
            sb.AppendLine("<td>");
